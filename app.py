...
def charger_donnees():
    ...
    return donnees.copy()  # Retourner une copie des données pour éviter de modifier l'objet original
...
def diagramme_circulaire(donnees):
    ...
    pourcentages = donnees['risques_encourus_par_le_consommateur'].value_counts(normalize=True) * 100
...
def diagramme_barres(donnees):
    # Sélectionner les 10 sous-catégories de produits avec le plus de risques
    top_10_sous_categories = donnees['sous_categorie_de_produit'].value_counts().nlargest(10).index
    
    # Filtrer les données pour inclure uniquement les 10 sous-catégories de produits sélectionnées
    donnees_filtrees = donnees[donnees['sous_categorie_de_produit'].isin(top_10_sous_categories)]
    
    # Créer le diagramme à barres avec un code couleur pour les risques encourus par le consommateur
    fig = px.bar(donnees_filtrees, 
                 x='sous_categorie_de_produit', 
                 color='risques_encourus_par_le_consommateur',
                 title="Top 10 des sous-catégories de produits avec les risques les plus élevés",
                 labels={'sous_categorie_de_produit': 'Produits Alimentaires', 'risques_encourus_par_le_consommateur': 'Risques'})
    
    # Afficher le graphique
    st.plotly_chart(fig, use_container_width=True)

# Titre de l'application
st.title('Top 10 des sous-catégories de produits avec les risques les plus élevés')

# Charger les données
donnees = charger_donnees()

# Afficher le diagramme à barres
diagramme_barres(donnees)
...
# Fonction pour extraire les dates de début et de fin de commercialisation
def extraire_dates_commercialisation(donnees):
    # Extraire les dates de début et de fin de commercialisation
    donnees['Date debut'] = pd.to_datetime(donnees['date_debut_fin_de_commercialisation'].str.extract(r'Du (\d{2}/\d{2}/\d{4})', expand=False), format='%d/%m/%Y')
    donnees['Date fin'] = pd.to_datetime(donnees['date_debut_fin_de_commercialisation'].str.extract(r'au (\d{2}/\d{2}/\d{4})', expand=False), format='%d/%m/%Y')
    # Remplacer les valeurs manquantes par "Depuis le" pour les produits sans date de fin
    donnees['Date fin'].fillna("Depuis le", inplace=True)
    return donnees

# Fonction pour créer le diagramme basé sur les dates de début et de fin de commercialisation
def diagramme_dates_commercialisation(donnees, annee_selectionnee):
    # Filtrer les données pour l'année sélectionnée
    donnees_filtrees = donnees[donnees['Date debut'].dt.year == annee_selectionnee]
    
    # Extraire le mois de la date de début de commercialisation
    donnees_filtrees['Mois'] = donnees_filtrees['Date debut'].dt.month_name()
    
    # Créer le diagramme à barres
    fig = px.histogram(donnees_filtrees, 
                       x='Mois', 
                       color='risques_encourus_par_le_consommateur',
                       title=f"Distribution des produits en fonction du mois de début de commercialisation pour l'année {annee_selectionnee}",
                       labels={'Mois': 'Mois de début de commercialisation', 'count': 'Nombre de produits'},
...
# Titre de l'application
st.title('Analyse des dates de début de commercialisation')
...
# Supprimer la variable "nan" des années disponibles
donnees = donnees[~donnees['Date debut'].dt.year.isna()]

# Obtenir la liste des années disponibles
annees_disponibles = sorted(donnees['Date debut'].dt.year.unique(), reverse=True)

# Supprimer les années avec une virgule
annees_disponibles = [int(annee) for annee in annees_disponibles if not isinstance(annee, float)]

# Menu déroulant pour choisir l'année
annee_selectionnee = st.selectbox("Choisir l'année:", annees_disponibles)

# Afficher le diagramme basé sur les dates de début et de fin de commercialisation
diagramme_dates_commercialisation(donnees, annee_selectionnee)
...
def page_diagrammes():
    ...
    if choix_diagramme == "Sous-catégories de produits avec Risques encourus par le consommateur":
        ...
        diagramme_barre_sous_cat(donnees, risque)
